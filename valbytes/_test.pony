...
class \nodoc\ iso _NumericReadableTest is UnitTest
  ...
  fun apply(h: TestHelper) ? =>
    ...
    let ba =
      ByteArrays([as U8: 1; 2; 3], [as U8: 4])

    h.assert_eq[U8](
      arr.read_u8(0)?, ba.read_u8(0)?)
    h.assert_eq[U8](
      arr.read_u8(1)?, ba.read_u8(1)?)
    h.assert_eq[U8](
      arr.read_u8(2)?, ba.read_u8(2)?)
    h.assert_eq[U8](
      arr.read_u8(3)?, ba.read_u8(3)?)
    h.assert_error({()? => ba.read_u8(4)? })

    h.assert_eq[U16](
      arr.read_u16(0)?, ba.read_u16(0)?)
    h.assert_eq[U16](
      arr.read_u16(1)?, ba.read_u16(1)?)
    h.assert_eq[U16](
      arr.read_u16(2)?, ba.read_u16(2)?)
    h.assert_error({()? => ba.read_u16(3)? })

    h.assert_eq[U32](
      arr.read_u32(0)?, ba.read_u32(0)?)
    h.assert_error({()? => ba.read_u32(1)? })

primitive \nodoc\ _ByteArrayAndSourceGen
  """
  Generator that returns a continuous byte array
  and a ByteArrays instance made from random
  splits of the first array.

  TODO: create a separate generator using
  non-consecutive source arrays.
  """
  fun apply(
    min_size: USize = 0,
    max_size: USize = 100)
    : Generator[(Array[U8] val, ByteArrays)]
  =>
    let size_gen =
    ...
    =
      size_gen
        ...
        =>
          ...
          Generators
            .zip2[Array[U8] iso, Array[USize] iso](
              array_gen,
              split_gen)
        })
    array_and_splits_gen
      .map[(Array[U8] val, ByteArrays)](
      {(arg: (Array[U8] iso, Array[USize] iso)) =>
        (let immutable_arr: Array[U8] val,
          let splits: Array[USize] val) =
            recover val consume arg end
        ...
        for split in (consume splits).values() do
          ...
          if running_sum >= immutable_arr.size() then
            break
          end
        end
        (immutable_arr, ba)
...
class \nodoc\ iso _SizeProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  fun gen(): Generator[(Array[U8] val, ByteArrays)] =>
    _ByteArrayAndSourceGen(where max_size=1000)

  fun property(
    sample: (Array[U8] val, ByteArrays),
    h: PropertyHelper)
  =>
    h.assert_eq[USize](
      sample._1.size(), sample._2.size())
...
class \nodoc\ iso _DropProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  =>
    h.assert_eq[USize](
      sample._1.size(), sample._2.drop(0).size())
    if sample._1.size() > 0 then
      h.assert_eq[USize](
        sample._1.size() - 1,
        sample._2.drop(1).size())
    end
    h.assert_array_eq[U8](
      sample._1, sample._2.drop(0).array())
    h.assert_eq[USize](
      0, sample._2.drop(sample._2.size()).size())
    h.assert_eq[USize](
      0,
      sample._2.drop(sample._2.size() + 1).size())
...
class \nodoc\ iso _TakeProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  =>
    h.assert_eq[USize](
      sample._1.size(),
      sample._2.take(sample._1.size()).size())
...
class \nodoc\ iso _SelectProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  fun gen(): Generator[(Array[U8] val, ByteArrays)] =>
    _ByteArrayAndSourceGen(where max_size=100)
  ...
  =>
    ...
    h.assert_array_eq[U8](
      sample._1, sample._2.select().array())
    let some = USize(10).min(sample_size)
    h.assert_array_eq[U8](
      sample._1.trim(0, some),
      sample._2.select(0, some).array())
    h.assert_array_eq[U8](
      sample._1.trim(some),
      sample._2.select(some, -1).array())
...
class \nodoc\ iso _TrimProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  =>
    ...
    h.assert_array_eq[U8](
      sample._1.trim(0, middle),
      sample._2.trim(0, middle))
    h.assert_array_eq[U8](
      sample._1.trim(middle),
      sample._2.trim(middle))
    h.assert_eq[USize](
      0, sample._2.trim(0, 0).size())
    h.assert_array_eq[U8](
...
class \nodoc\ iso _ReadNumericProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  fun gen(): Generator[(Array[U8] val, ByteArrays)] =>
    _ByteArrayAndSourceGen(where min_size=16)
  ...
  =>
    h.assert_eq[U8](
      ...
      sample._2.read_u8(0)?)
    h.assert_eq[U16](
      sample._1.read_u16(0)?,
      sample._2.read_u16(0)?)
    h.assert_eq[U32](
      sample._1.read_u32(0)?,
      sample._2.read_u32(0)?)
    h.assert_eq[U64](
      sample._1.read_u64(0)?,
      sample._2.read_u64(0)?)
    h.assert_eq[U128](
...
class \nodoc\ iso _SkipProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  fun gen(): Generator[(Array[U8] val, ByteArrays)] =>
    _ByteArrayAndSourceGen(
      where min_size=2, max_size=1000)
...
class \nodoc\ iso _AddProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  fun property(
    ...
    h: PropertyHelper)
  =>
    let add_me: Array[U8] val =
      [as U8: 'A'; 'B'; 'C'; 'D']
    let added_array =
      recover val
        sample._1.clone() .> append(add_me)
      end
...
class \nodoc\ iso _ArraysProperty is Property1[(Array[U8] val, ByteArrays)]
  ...
  =>
    ...
    let acc: Array[U8] iso =
      recover iso Array[U8](sample._1.size()) end
    for array in sample._2.arrays().values() do
      acc.append(array)
    end
    h.assert_array_eq[U8](
      sample._1, consume acc)
...
class \nodoc\ iso _SipHash24StreamingProperty is Property1[Array[U8]]
  ...
  =>
    ...
    let array_hash =
      SipHash24.apply[Array[U8]](sample)
    h.assert_eq[U64](
...
class \nodoc\ iso _HalfSipHash24StreamingProperty is Property1[Array[U8]]
  ...
  =>
    ...
    let streaming_hash = sip.finish()
    let array_hash =
      HalfSipHash24.apply[Array[U8]](sample)
    h.assert_eq[U32](
      array_hash, streaming_hash)
